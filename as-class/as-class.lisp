...
(defmacro defstruct* (&body body)
  (multiple-value-bind(name options documentation slots)(parse body)
    (check options)
    (let((slots(slots slots options name)))
      `(PROGN
...
(macrolet((!(form)
  ...
  (defun enslot(slot conc-name) ; separated cause of huge.
    (let((null(make-symbol "NULL")))
      (labels((PARSE-SLOT(slot) ; slot is already canonicalized.
		(values (car slot)
			(cadr slot)
			(cddr slot)))
  ...
  (let((constructors(collect-option :constructor options)))
    (labels((DEFAULT-CONSTRUCTOR(constructor)
	    ...
	    (CANONICALIZE(lambda-list)
	      (let(acc)
		(dolist(var (Vars<=lambda-list lambda-list)
			    (nreverse acc))
		  (push (intern(symbol-name var):keyword)acc)
		  (push var acc))))
	    (DO-RETURN(acc)
	      (nreverse acc))
	    )
